...
public class AlgorytmBruteForce
{
    ...
    private StatisticsAlgorithmsHelper stHelper;

    public AlgorytmBruteForce( Graf graf )
    {
        this.graf = graf;
        stHelper = new StatisticsAlgorithmsHelper( CommonVariables.ALGORITHM_BRUTE_FORCE );
    }
    ...
    public void computeZlozonePamieciowo()
    {
        ...
        // znajdz najmniejsza liczbe elementow sposrod zbiorow dajacych
        // acykliczny graf
        int min = 0;
    ...
    public StringBuilder compute()
    {
        ...
        // zaczynamy od zbiorow 0-elementowych (pusty), pozniej wszystkich
        // 1-elementowych itp..
        ...
        // START POMIARÓW !!!!!
        // -----------
        stHelper.startCalculateComplexity();

        // dla kazdej mozliwej liczby wierzcholkow w podzbiorze k, gdzie k = 0,
        // 1, ... ilosc_wierzcholkow
        for ( int i = 0; i < ( this.graf.getWierzcholki().size() ); i++ )
        {
            // dla kazdej liczby elementow w podzbiorze tworzymy nowy zbior
            // acyklicznych zbiorow
            ...
            // dla kazdego k-elementowego podzbioru, gdzie k = 0, 1, ...
            // ilosc_wierzcholkow
            for ( long j = 0; j < iloscMozliwychPodzbiorowKElementowych; j++ )
            {

                // oblicz maske na podstawie wygenerowanej permutacji. Metoda,
                // ktora pozyskuje ustawione bity na podstawie
                // subsetPermutationMask
                ...
                // dla 0 istnieje tylko zbior pusty, wiec nie ma potrzeby
                // generowania jego permutacji
        ...
        if ( !znalezionoFVS )
        {
            results.append( "Zbiór [FVS] pokrywa siê ze wszystkimi " + liczbaElementowPodzbioru
                            + " wierzcho³kami grafu i jest nastêpuj¹cy: \n" );
            results.append( this.graf.getWierzcholki() );
            System.out.println( "Zbiór [FVS] pokrywa siê ze wszystkimi " + liczbaElementowPodzbioru
                                + " wierzcho³kami grafu i jest nastêpuj¹cy: " );
    ...
    public Set<Wierzcholek> pobierzZbiorWierzcholkowNaPodstawieMaski( boolean[] maska )
    {
        ...
        // najpierw sprawdzamy czy nie mamy do czynienia ze zbiorem pustym -
        // jesli tak, zwracamy taki
